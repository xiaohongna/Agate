...
CComPtr<ID2D1Device1>           m_d2dDevice;
CComPtr<ID2D1DeviceContext1>    m_d2dContext;
...
HRESULT InitID2D1DeviceContext(HWND hwnd);
HRESULT CreateBitmapFromFile(PCWSTR uri, ID2D1Bitmap** ppBitmap);
void Draw();
...
HRESULT InitID2D1DeviceContext(HWND hwnd)
{
   ...
   // You need it for compatibility with Direct2D.
	...
	auto hr = m_d2dFactory->CreateDevice(dxgiDevice, &m_d2dDevice);
	...
	dxgiFactory->CreateSwapChainForHwnd(device,
		hwnd,
		&swapChainDesc,
		nullptr,
		nullptr,
	...
	m_d2dContext->CreateBitmapFromDxgiSurface(dxgiBackBuffer,&bitmapProperties,&m_d2dTargetBitmap);
...
HRESULT CreateBitmapFromFile(PCWSTR uri,ID2D1Bitmap** ppBitmap)
{
	...
	CComPtr<IWICBitmapScaler> pScalar;
	...
	if (SUCCEEDED(hr))
	{
		hr = m_d2dContext->CreateBitmapFromWicBitmap(pConverter, NULL, ppBitmap);
	}
	return hr;
}
